...
const moveStarts: Array<{ paths: string[]; target_dir: string }> = [];
...
describe("move removes moved rows and reports partial failure separately", () => {
  it("moves selected files and keeps failed targets visible", async () => {
    installBackendMock();
    render(<App />);
    ...
    expect(moveStarts[0].paths).toEqual(["/music/a.mp3", "/music/b.mp3"]);
    expect(moveStarts[0].target_dir).toBe("/library");

    // First file moves; second collides (partial failure).
    await waitFor(() => {
    ...
    expect(within(summary).getByText("1 file moved.")).toBeInTheDocument();
    expect(
      within(summary).getByText("1 file could not be moved:"),
    ).toBeInTheDocument();
  });
});
