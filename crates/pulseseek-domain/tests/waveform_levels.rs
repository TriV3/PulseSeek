...
fn waveform(levels: Vec<Level>) -> MultiresolutionWaveform {
    MultiresolutionWaveform::from_levels(levels).expect("valid waveform")
}
...
fn waveform_empty_rejected() {
    let err = MultiresolutionWaveform::from_levels(vec![]).expect_err("empty rejected");
...
fn waveform_non_contiguous_indices_rejected() {
    ...
    let err = MultiresolutionWaveform::from_levels(levels).expect_err("gap rejected");
...
fn waveform_starting_above_zero_rejected() {
    ...
    let err =
        MultiresolutionWaveform::from_levels(levels).expect_err("not starting at zero rejected");
    assert_eq!(err, WaveformError::NonContiguousIndices);
}
...
fn waveform_level_with_zero_peaks_rejected() {
    ...
    let err = MultiresolutionWaveform::from_levels(levels).expect_err("zero peaks rejected");
...
fn waveform_level_with_zero_samples_per_peak_rejected() {
    ...
    let err =
        MultiresolutionWaveform::from_levels(vec![lvl]).expect_err("zero resolution rejected");
...
fn waveform_equal_resolution_rejected() {
    ...
    let err = MultiresolutionWaveform::from_levels(levels).expect_err("equal resolution rejected");
...
fn waveform_finer_level_with_more_samples_per_peak_rejected() {
    ...
    let err =
        MultiresolutionWaveform::from_levels(levels).expect_err("increasing resolution rejected");
    assert_eq!(err, WaveformError::NonDecreasingResolution);
}

#[test]
fn waveform_valid_pyramid_accepted() {
    let w = waveform(vec![level(0, 16, 4), level(1, 8, 8), level(2, 4, 16)]);
    assert_eq!(w.len(), 3);
    assert!(!w.is_empty());
}
...
fn waveform_single_level_accepted() {
    let w = waveform(vec![level(0, 4, 16)]);
    assert_eq!(w.len(), 1);
}
...
fn waveform_coarsest_is_first() {
    let w = waveform(vec![level(0, 16, 4), level(1, 8, 8)]);
...
fn four_eight_sixteen() -> MultiresolutionWaveform {
    waveform(vec![level(0, 16, 4), level(1, 8, 8), level(2, 4, 16)])
}
